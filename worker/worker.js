  ...
  async fetch(request, env) {
    ...
    if (request.method === 'GET' && url.pathname === '/giphy-search') {
      try {
        ...
        const gifs = (giphyData.data || []).map(g => ({
          id: g.id,
          title: g.title,
        ...
        return new Response(JSON.stringify({ success: true, gifs }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        });
      } catch (err) {
        return new Response(JSON.stringify({ error: err.message || 'Giphy fetch failed' }), {
          status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        });
      }
    }
